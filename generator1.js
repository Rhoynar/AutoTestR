...
if (!document.createTreeWalker) { // assuming TreeWalker
    ...
    function setDefinitions(input) {
        //console.log("set def url :"+URL);
        var definitions = [];
        //var root = document.querySelector(input.nodes.root) || document;
        var root = input.documentElement;
       ...
       // }
        ...
        var tags = input.getElementsByTagName('*');
        var texts = {};
        var unsets = {};
        ...
        if (submit.text !== '' && input.operations.extras.submit) {
            //console.log(input.model.target);
            var buffer = {
                attribute: {},
                operation: {
    ...
    return {
        generate: function(input, common1, myURL) {
            //console.log(common1);
            URL = myURL;
            input = input || {};
            var output = Object.extend(input);

            output = common1.setDefaultValues(output);
            output = setDefinitions(output);
            //output.url = input.location.href;
            output.url = myURL;
            //console.log("this is: "+myURL);
...
exports.startGenerator = function(input, common1, myURL){
    document = input;
    //console.log(myURL);
    common1.URL = myURL;
    //console.log("url is: "+common1.URL);
    return POG.generate(input, common1, myURL);
};
